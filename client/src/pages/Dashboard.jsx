...
const Dashboard = () => {
  ...
  //create resume in the database and also the dashboard
  ...
  //upload resume by the cloud and also to import existing resume..
  ...
  //delete resume by the help of function
  ...
  return (
    <div>
      <div className='max-w-7xl mx-auto px-4 py-8'>
        <p className='text-2xl font-medium mb-6 bg-linear-to-r from-slate-600 to-slate-700 bg-clip-text text-transparent sm:hidden'>welcome ,Raj</p>
        ...
        <div className='grid grid-cols-2 sm:flex flex-wrap gap-4'>
          {allResumes.map((resume, idx) => {
            const baseColour = colors[idx % colors.length]
            return (
              <button key={idx} onClick={() => navigate(`/app/builder/${resume._id}`)} className='group relative w-full sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 border group-hover:shadow-lg transition-all duration-300 cursor-pointer' style={{ background: `linear-gradient(135deg,${baseColour}10,${baseColour}40)`, borderColor: baseColour + '40' }}>

                <FilePenLineIcon className='size-7 group-hover:scale-105 transition-all' style={{ color: baseColour }} />
                <p className='text-sm group-hover:scale-105 transition-all px-2 text-center' style={{ color: baseColour }}>
                  {resume.title}
                </p>
                <p className='absolute bottom-1 text-[11px] text-slate-400 group-hover:text-slate-500 transition-all duration-300 px-2 text-center' style={{ color: baseColour + '90' }}>
        ...
        {showUploadResume && (
          <form onSubmit={uploadResume} onClick={() => setShowUploadResume(false)} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
            <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
              ...
              <button disabled={isLoading} className='w-full py-2 bg-green-600 text-white rounded hover:bg-green-700 transition-colors disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2'>
