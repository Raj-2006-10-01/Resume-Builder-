...
const PersonalinfoForm = ({ data, onChange, removeBackground, setRemoveBackground }) => {
    ...
    const fields = [
        ...
        { key: "linkedin", label: "Linkdin Profile", icon: Link, type: "url" },
            ...
            {fields.map((field) => {
                ...
                return (
                    <div key={field.key} className='space-y-1 mt-5'>
                        ...
                        <input type={field.type} value={data[field.key] || ""} onChange={(e) => handleChange(field.key, e.target.value)} className='mt-1 w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring focus:ring-blue-500 focus:border-blue-500 outline-none text-sm' placeholder={`Enter Your ${field.label.toLowerCase()}`} required={field.required} />
                    </div>
                )
            })}
        </div>
